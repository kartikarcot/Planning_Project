...
static const double kdPI = 3.14159;

inline double Distance(const std::vector<double> one, const std::vector<double> two)
{
    std::size_t N = one.size();
    double result = 0.0;
    for (std::size_t i = 0; i < N; ++i) {
        double e = one[i] - two[i];
        if (e > kdPI) {
            e -= 2 * kdPI;
        } else if (e < -kdPI) {
            e += 2 * kdPI;
...
public:
    ...
    KDTree(std::size_t N, std::vector<std::pair<std::vector<double>, ElemType>>& points);
     ...
     */
    void insert(const std::vector<double>& pt, const ElemType& value=ElemType());
     ...
     */
    ElemType& operator[](const std::vector<double>& pt);

    /*
     * Returns a reference to the key associated with the point pt. If the point
     * is not in the tree, this function throws an out_of_range exception.
     */
    ElemType& at(const std::vector<double>& pt);
    const ElemType& at(const std::vector<double>& pt) const;
     ...
     */
    ElemType kNNValue(const std::vector<double>& key, std::size_t k) const;

    /*
     * Returns the nearest neightbor to pt
     */
    ElemType nearestNeighbor(const std::vector<double>& pt) const;
...
private:
    struct Node {
        ...
        ElemType value;
        Node(const std::vector<double>& _pt, int _level, const ElemType& _value=ElemType()):
     ...
     */
    Node* buildTree(typename std::vector<std::pair<std::vector<double>, ElemType>>::iterator start,
                    typename std::vector<std::pair<std::vector<double>, ElemType>>::iterator end, int currLevel);
     ...
     */
    Node* findNode(Node* currNode, const std::vector<double>& pt) const;

    // Recursive helper method for kNNValue(pt, k)
    void nearestNeighborRecurse(const Node* currNode, const std::vector<double>& key, BoundedPQueue<ElemType>& pQueue) const;
...
template <typename ElemType>
KDTree<ElemType>::KDTree(std::size_t N) :
    root_(NULL), size_(0), N(N) { }
...
template <typename ElemType>
typename KDTree<ElemType>::Node* KDTree<ElemType>::buildTree(typename std::vector<std::pair<std::vector<double>, ElemType>>::iterator start,
                                                             typename std::vector<std::pair<std::vector<double>, ElemType>>::iterator end, int currLevel) {
    if (start >= end) return NULL; // empty tree

    int axis = currLevel % N; // the axis to split on
    auto cmp = [axis](const std::pair<std::vector<double>, ElemType>& p1, const std::pair<std::vector<double>, ElemType>& p2) {
...
template <typename ElemType>
KDTree<ElemType>::KDTree(std::size_t N, std::vector<std::pair<std::vector<double>, ElemType>>& points) {
...
template <typename ElemType>
typename KDTree<ElemType>::Node* KDTree<ElemType>::findNode(typename KDTree<ElemType>::Node* currNode, const std::vector<double>& pt) const {
    if (currNode == NULL || currNode->point == pt) return currNode;

    const std::vector<double>& currPoint = currNode->point;
...
template <typename ElemType>
bool KDTree<ElemType>::contains(const std::vector<double>& pt) const {
    auto node = findNode(root_, pt);
    return node != NULL && node->point == pt;
}

template <typename ElemType>
void KDTree<ElemType>::insert(const std::vector<double>& pt, const ElemType& value) {
    assert(pt.size() == N);
...
template <typename ElemType>
const ElemType& KDTree<ElemType>::at(const std::vector<double>& pt) const {
    auto node = findNode(root_, pt);
...
template <typename ElemType>
ElemType& KDTree<ElemType>::at(const std::vector<double>& pt) {
    const KDTree<ElemType>& constThis = *this;
    return const_cast<ElemType&>(constThis.at(pt));
}

template <typename ElemType>
ElemType& KDTree<ElemType>::operator[](const std::vector<double>& pt) {
    auto node = findNode(root_, pt);
...
template <typename ElemType>
void KDTree<ElemType>::nearestNeighborRecurse(const typename KDTree<ElemType>::Node* currNode, const std::vector<double>& key, BoundedPQueue<ElemType>& pQueue) const {
    if (currNode == NULL) return;
    const std::vector<double>& currPoint = currNode->point;
...
template <typename ElemType>
ElemType KDTree<ElemType>::kNNValue(const std::vector<double>& key, std::size_t k) const {
...
template <typename ElemType>
ElemType KDTree<ElemType>::nearestNeighbor(const std::vector<double>& key) const {
    return this->kNNValue(key, 1);
}
