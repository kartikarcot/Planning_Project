...
def get_path(x0, x1):
    DIR = os.path.realpath(os.path.dirname(os.path.realpath(__file__)) + "/../CVAE")
    MODEL_DIR = DIR + "/Models/Unified"
    # mini_map_file = os.path.join(DIR+"/Training_Data/", 'map{}_mini.npy'.format(MAP_NUM))
    mini_map_file = DIR + "/Training_Data/map5_mini.npy"
    mini_map = np.load(mini_map_file)
    # map_file = os.path.join(DIR+"/Training_Data/", 'map{}.npy'.format(MAP_NUM))
    map_file = os.path.join(DIR + "/Training_Data/", 'map{}.npy'.format(5))
    ...
    planner.initialize(np.array([x0[0], x0[1], x0[2]]), np.array([x1[0], x1[1], x1[2]]), np.array([0, 0, 0]),
...
if __name__ == "__main__":
    ...
    sampler.initialize(MODEL_DIR)
    planner = FMT_Star(3, 500, sampler.sample, checker.is_in_collision)
    planner.initialize(np.array([10 / 160, 10 / 160, 0]), np.array([30 / 160, 110 / 160, 0]), np.array([0, 0, 0]),
                       np.array([1, 1, 2 * np.pi]))
    plt.scatter(x=160 * planner.points[:, 1], y=160 * planner.points[:, 0], color='red', s=2)
    plt.imshow(_map)
    plt.show()
